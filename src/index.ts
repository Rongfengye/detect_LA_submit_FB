...
const provider = new providers.WebSocketProvider(
    "wss://speedy-nodes-nyc.moralis.io/5b4f2959281f08d79d7cf28b/eth/rinkeby/ws",
    "rinkeby",
);
...
async function main() {
    ...
    console.log(
        "Hitting the Websocket endpoint at wss://speedy-nodes-nyc.moralis.io/5b4f2959281f08d79d7cf28b/eth/rinkeby/ws",
    );
    ...
    const handle = provider.on("pending", async pending_tx_hash => {
        // console.log("New Pending Transction with Hash", pending_tx_hash);
        console.log("New Pending Transaction", num);
        num++;

        const pending_tx = await provider.getTransaction(pending_tx_hash);
